  ...
  return (
    <div className="jScroll" style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 74, paddingTop: 52, overflow: 'auto' }}>
      <div style={{ padding: '14px 20px 6px' }}>
        ...
        <div style={{ fontSize: 12, color: '#8A7C68', marginTop: 4 }}>
      ...
      <div onClick={() => setNotice(n => !n)} style={{ margin: '10px 16px 14px', borderRadius: 22, background: '#201812', padding: '15px 18px', display: 'flex', alignItems: 'center', gap: 13, position: 'relative', overflow: 'hidden', cursor: 'pointer' }}>
        ...
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <div style={{ color: 'rgba(255,255,255,.62)', fontSize: 11.5, marginTop: 1 }}>{notice ? "payments aren't set up in this build yet" : 'jodi gold · unblur every crush'}</div>
        </div>
      ...
      {app.likesLoading && (
        <div style={{ textAlign: 'center', color: '#8A7C68', fontSize: 12.5, marginTop: 30 }}>loading…</div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 13, padding: '0 16px 20px' }}>
        {app.likesYou.map((lk, i) => (
          <div
            key={lk.user.id}
            data-testid={i === 0 ? 'crush-card-first' : 'crush-card-blurred'}
            onClick={() => { if (i === 0) app.likeBack(lk.user.id); else setNotice(true) }}
            style={{ position: 'relative', borderRadius: '88px 88px 20px 20px', overflow: 'hidden', aspectRatio: '3/4', background: lk.user.grad, border: '2px solid #201812', cursor: 'pointer' }}
          >
            ...
            <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '24px 12px 11px', background: 'linear-gradient(to top, rgba(18,13,8,.85), transparent)' }}>
              <div style={{ color: '#fff', fontSize: 12.5, fontWeight: 700 }}>{lk.user.chips.slice(0, 2).join(' · ') || 'new to jodi'}</div>
              <div style={{ color: 'rgba(255,255,255,.72)', fontSize: 10.5, marginTop: 1 }}>{lk.user.city || (i === 0 ? 'tap to say hi back' : 'unlock to reveal')}</div>
            </div>
            {lk.rose && (
      ...
      {!app.likesLoading && app.likesYou.length === 0 && (
        <div style={{ textAlign: 'center', color: '#8A7C68', fontSize: 12.5, padding: '10px 40px 30px', lineHeight: 1.5 }}>
          nobody's said hi yet — swipe through today's five and check back later.
        </div>
      )}
    </div>
  )
