...
function VoicePill({ url, label }: { url: string; label: string }) {
  const { playing, toggle } = useAudioPlayer(url)
  return (
    <div
      onClick={e => { e.stopPropagation(); toggle() }}
      onPointerDown={e => e.stopPropagation()}
      style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: 'rgba(255,255,255,.16)', border: '1px solid rgba(255,255,255,.26)', padding: '4px 11px 4px 5px', borderRadius: 22, cursor: 'pointer' }}
    >
      <div style={{ width: 22, height: 22, borderRadius: '50%', background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        {playing
          ? <svg width="8" height="9" viewBox="0 0 9 11"><rect x="0" y="0" width="3" height="11" fill="#E5326E" /><rect x="6" y="0" width="3" height="11" fill="#E5326E" /></svg>
          : <svg width="8" height="9" viewBox="0 0 9 11"><path d="M0 0l9 5.5L0 11z" fill="#E5326E" /></svg>}
      </div>
      ...
      <span style={{ fontSize: 10.5, color: '#fff', fontWeight: 700 }}>{label}</span>
    </div>
  )
}
...
export default function DiscoverScreen({ app }: { app: JodiApp }) {
  ...
  const photoUrl = app.profile?.photos[0]?.url
  ...
  return (
    <div style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 74, display: 'flex', flexDirection: 'column', paddingTop: 52 }}>
      ...
      {app.deckLoading && (
        <div style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 13, color: '#8A7C68' }}>finding your five…</div>
      )}
      ...
      {!app.deckLoading && app.hasDeck && app.profile && (
        <div style={{ flex: 1, position: 'relative', margin: '0 18px 0' }}>
          ...
          >
            <div style={{ position: 'absolute', left: 0, right: 0, top: 0, bottom: 74, borderRadius: '162px 162px 32px 32px', overflow: 'hidden', background: photoUrl ? '#120d08' : app.profile.grad, backgroundImage: photoUrl ? `url(${photoUrl})` : undefined, backgroundSize: 'cover', backgroundPosition: 'center', border: '2.5px solid #201812', boxShadow: '0 20px 40px rgba(32,24,18,.22)', userSelect: 'none' }}>
              {!photoUrl && (
                <>
              ...
              <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, padding: '60px 18px 16px', background: 'linear-gradient(to top, rgba(18,13,8,.93), rgba(18,13,8,.45) 58%, transparent)' }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
                  <span data-testid="card-name" style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 800, fontSize: 30, color: '#fff', lineHeight: 1, letterSpacing: '-1.1px' }}>{app.profile.name}</span>
                ...
                {app.profile.prompts[0] && (
                  <div style={{ marginTop: 10, background: '#FBF3E4', borderRadius: 16, padding: '10px 13px', border: '2px solid #201812' }}>
                    <div style={{ fontSize: 9.5, color: '#E5326E', fontWeight: 800, letterSpacing: '.5px', textTransform: 'uppercase' }}>{app.profile.prompts[0].question}</div>
                    <div style={{ fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 600, fontSize: 15, color: '#201812', marginTop: 3, lineHeight: 1.3, letterSpacing: '-.3px' }}>{app.profile.prompts[0].answer}</div>
                  </div>
                )}
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginTop: 9 }}>
                  {app.profile.voiceUrl && <VoicePill url={app.profile.voiceUrl} label={fmtDuration(app.profile.voiceDurationSec)} />}
                  <span style={{ fontSize: 10.5, color: 'rgba(255,255,255,.62)', fontWeight: 600 }}>tap for more →</span>
                </div>
              </div>
            </div>

            <div style={{ position: 'absolute', left: 0, right: 0, bottom: 2, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 14 }} onPointerDown={e => e.stopPropagation()} onPointerUp={e => e.stopPropagation()}>
              ...
              <div data-testid="btn-pass" onClick={app.flingPass} style={{ width: 56, height: 56, borderRadius: '50%', background: '#FFFDF7', border: '2px solid #201812', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '2.5px 2.5px 0 #201812', cursor: 'pointer' }}>
              ...
              <div data-testid="btn-rose" onClick={app.sendRose} style={{ width: 56, height: 56, borderRadius: '50%', background: '#16A6A0', border: '2px solid #201812', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '2.5px 2.5px 0 #201812', cursor: 'pointer' }}>
              ...
              <div data-testid="btn-like" onClick={app.flingLike} style={{ width: 66, height: 66, borderRadius: '50%', background: '#E5326E', border: '2px solid #201812', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '3px 3px 0 #201812', cursor: 'pointer' }}>
      ...
      {app.deckEnded && (
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 42px', textAlign: 'center', gap: 13, position: 'relative' }}>
          ...
          <div style={{ fontSize: 13, color: '#8A7C68', lineHeight: 1.5, position: 'relative' }}>five people worth meeting &gt; five hundred to scroll past. check back once more of the community joins, or run today's five back.</div>
