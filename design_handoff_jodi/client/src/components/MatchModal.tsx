  ...
  return (
    <div style={{ position: 'absolute', inset: 0, zIndex: 70, background: '#E5326E', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '0 32px', textAlign: 'center', overflow: 'hidden', animation: 'jFade .28s ease' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', marginTop: 32, position: 'relative' }}>
        <div style={{ width: 96, height: 118, borderRadius: '48px 48px 16px 16px', background: me.grad, border: '2.5px solid #201812', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Bricolage Grotesque', sans-serif", fontWeight: 800, fontSize: 36, color: '#fff', animation: 'jAvL .6s cubic-bezier(.2,1.3,.4,1) both', zIndex: 2 }}>{me.mono}</div>
      ...
      <div style={{ fontSize: 15, color: 'rgba(255,255,255,.95)', marginTop: 26, lineHeight: 1.5, fontWeight: 500, animation: 'jRise .6s .15s ease both', position: 'relative' }}>
        you and {matchProfile.name} both said hi.<br />break the ice.
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11, marginTop: 26, width: '100%', animation: 'jRise .6s .25s ease both', position: 'relative' }}>
        <div data-testid="match-start" onClick={app.messageMatch} style={{ padding: 16, borderRadius: 28, background: '#F5A524', border: '2.5px solid #201812', boxShadow: '3px 3px 0 #201812', color: '#201812', fontSize: 15.5, fontWeight: 800, cursor: 'pointer' }}>start the convo</div>
        <div data-testid="match-keep" onClick={app.closeMatch} style={{ padding: 14, borderRadius: 28, background: 'transparent', color: '#fff', fontSize: 14, fontWeight: 700, cursor: 'pointer', border: '2px solid rgba(255,255,255,.55)' }}>keep looking</div>
      </div>
    </div>
  )
}
